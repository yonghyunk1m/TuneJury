...
| `popularity_probe.py` | Appendix F (Amateur-vs-professional probe) | FMA-Large (106K tracks): bottom listens decile $\to$ $-1.413$ reward, top decile $\to$ $+0.084$, gap $\sim 1.50$ reward units. Spearman(log-listens, reward) $=+0.285$, $p<0.001$. |
...
- `release_scores/fma_large_scores_rescored.csv`: 106K pre-computed per-track
...
| --- | --- | --- |
| `per_system_ranking.py` | `model_a`, `model_b` per pair | The original Music Arena / AIME / MusicPrefs preference dumps that fed `data/extract_features.py`. The companion `music-ranknet` preprocessing repo emits these as `<DS>/json/<uuid>.json`. |
